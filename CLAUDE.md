...
ユーザーが **「今日は終わりにしよう」** または同等の終了宣言（「今日はここまで」「終わり」など明確に作業終了を示す発言）を行ったら、以下を順に実行する：

1. **`progress.md` を更新**: 今日のセッションでやったことを「今日やったこと」セクションにまとめ、古い情報は削除して最新状態に書き換える。日付ヘッダも今日の日付に更新
2. **git にコミット**: 変更をすべて `git add -A` でステージし、内容に合わせた日本語コミットメッセージで commit。フッターに `Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>` を含める
3. **GitHub にプッシュ**: `git push origin main`。HTTP 400 が出たら `http.postBuffer` 設定済みのはず（500MB）

ユーザーが明示的にコミット/プッシュ不要と言った場合のみスキップする。
